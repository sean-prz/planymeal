...
function RecipeList({ recipes }: RecipeListProps) {
  ...
  const [recipesPlanned, setRecipesPlanned] = useState<boolean[]>(
    recipes.map(() => false),
  );
  ...
  async function handleChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    setRecipesVisible(
      recipes.filter((recipe) => recipe.title.includes(input.toLowerCase())),
    );
  ...
  async function addPlannedRecipesToShoppingCard() {
    ...
    await repo.makeRecipes(recipes.filter((_, index) => recipesPlanned[index]));
    route.push("/shopping");
  }

  function setPlanned(index: number): (stat: boolean) => void {
    return (stat: boolean) => {
  ...
  useEffect(() => {
    async function getIngredients() {
      const repo = await SupaBaseRecipesRepository.getInstance();
      const ingre = await repo.getAllIngredients();
  ...
  return (
    <div>
      <div className="flex items-center m-5">
        ...
        <div
          className="px-4 inline-flex text-center"
          onClick={() => addPlannedRecipesToShoppingCard()}
        >
          {recipesPlanned.filter((it) => it).length}
          <br /> Selected
        </div>
        <p
      ...
      {showTextInput ? (
        <CommandPaletteInput
          ...
          suggestions={ingredients!.map((ingredient) => ingredient.name)}
